...
use crate::tournamentsim::fuzzer_inputs::{Action, FuzzState};
...
fn run_workload(input: &WorkloadInput) -> WorkloadReport {
    let pool = build_database();
    let app = crate::config::create_app(pool.clone());
    let mut client = TestServer::new(app).unwrap();
...
fn assert_tournament_properties(
